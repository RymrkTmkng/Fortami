        ...
        <div class="row rounded-3 shadow bg-dark bg-gradient text-center">
            ...
            ?>
                <div class="col-12 p-2 text-success">
                    <h2><i class="bi bi-plus-circle"> Add Method</i></h2>
                </div>
                <div class="col-12">
                    <form action="">
            ...
            ?>
                <div class="col-12 p-2 text-warning">
                    <h2><i class="bi bi-pencil-fill"> Edit Method</i></h2>
                </div>
                <div class="col-12">
                    <form action="">
                        <input type="hidden" name="method_id" value="<?=$_GET['id']?>" >
                        <input type="text" name="method" class="form-control" value="<?=$_GET['name']?>" placeholder="Method Name" required>
                        <button type="submit" name="editbtn" value="edit" class="btn btn-success form-control my-3">Save</button>
                        <a href="settings-other.php" class="btn btn-danger form-control">Cancel</a>
                    </form>
                </div>
            <?php
                    ...
                    elseif ($_GET['btn'] == 'del') {
                        $backend->delMethod($_GET['id']);
                    }
                }
                if (isset($_GET['savebtn'])) {
                    $method_name = $_GET['method'];
                    $backend->addMethod($method_name);
                 }
                 elseif (isset($_GET['editbtn'])) {
                    $method_name = $_GET['method'];
                    $id = $_GET['method_id'];
                    $backend->editMethod($method_name,$id);
                 }
            ?>
        </div>
    </div>
